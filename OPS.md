...
- **На сервері:** код `29a9895` (P1–P8, P5a, перенесення журналу, виправлення зависання, логіста й «відрізок»; розгорнуто 21.09, §6). Сайт `active`, recorder `active`/`enabled` (з 20.09 не перезапускався, його код не змінювався). Ledger — `0006_audit_actor`. Нерозгорнутого немає.
...
- Робоча копія `~/Projects/backend-journal-migration` (git worktree, гілка `journal-migration`, влита) — прибрати з дозволу власника.
